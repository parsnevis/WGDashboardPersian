"""
< Wireguard Panel > - Copyright(C) 2022 Mostafa Shoa BY [https://github.com/parsnevis]
...
# def get_peers(config_name, search="", sort_t="status", order_by_t="asd"):
def get_peers(config_name, search="", sort_t="status"):
    ...
    # reverse = "false" if order_by_t == "asd" else "true"
...
def create_peer_download_name(config_name, peer_name):
    """
    Get a peer name and create fuly download name
    @param config_name: Configuration Name
    ...
    if len(filename) == 0:
        ...
        # filename = config_name.capitalize() + filename.upper()
        filename = filename
...
def get_conf(config_name):
    ...
    conf_data = {
        # "peer_data": get_peers(config_name, search, sort, order_by),
...
def add_peer_bulk(config_name):
    ...
    last_record = g.cur.execute("SELECT * FROM " + config_name + " ORDER BY created_at DESC LIMIT 1").fetchall()[0]
    last_name = last_record[4]
    last_name = last_name.split('PT')
    count = 1
    for i in range(amount):
        ...
        number = int(last_name[1]) + count
        if(number < 10):
            name = f"{config_name.capitalize()}PT0{number}"
        else:
            name = f"{config_name.capitalize()}PT{number}"
        ...
        dtime = datetime.now()
        new_date = dtime + relativedelta(months=1)
    ...
    try:
        ...
        return jsonify({"message": "true", "data": file_name})
...
def add_peer(config_name):
    ...
    if 'bandwidth' in data:
        ...
        bandwidth = 0
    # bandwidth = float(data['bandwidth']) * pow(1024, 3) if 'bandwidth' in data else 100 * pow(1024, 3)
    # return f"{ends_at}--1"
    ...
    try:
        ...
        g.cur.execute(sql, (
            name, data['private_key'], data['DNS'], endpoint_allowed_ip, bandwidth, ends_at, 0, time.time(),
            public_key))
        
        # config.set("Peers", "count", count)
        # set_dashboard_conf(config)
        # config.clear()
...
def generate_qrcode(config_name):
    ...
    if len(get_peer) == 1:
        ...
        if peer[0] != "":
            ...
            filename = create_peer_download_name(config_name, peer[7])
            ...
            return render_template("qrcode.html", image=result, text=filename)
    else:
...
# Turn on / off a peer
@app.route('/switch_peer/<config_name>', methods=['GET'])
def switch_peer(config_name):
    """
    On/off the wireguard peer.
    @param config_name: Name of WG interface
    @type config_name: str
    @return: redirects
    """
    ...
    peer_id = request.args.get('id')
    peer = g.cur.execute("SELECT id, name, end_active, allowed_ip FROM " + config_name + " WHERE id = ?", (peer_id,)).fetchall()
    
    status = peer[0][2]
    allowed_ip = peer[0][3]
    # return jsonify(status, allowed_ip)
    
    # deactivation
    if status == 1:
        status = 0
        try:
            output = subprocess.check_output(f"wg set {config_name} peer {peer_id} remove", shell=True, stderr=subprocess.STDOUT).decode('UTF-8')
            ...
            subprocess.check_output(f'wg-quick save {config_name}', shell=True, stderr=subprocess.STDOUT)
            g.cur.execute("UPDATE %s SET end_active=%s WHERE id='%s'" % (config_name, status, peer_id))
        except subprocess.CalledProcessError as exc:
            session["switch_msg"] = exc.output.strip().decode("utf-8")
            return redirect('/')
    # activation
        ...
        status = 1
        try:
            output = subprocess.check_output(f"wg set {config_name} peer {peer_id} allowed-ips {allowed_ip}", shell=True, stderr=subprocess.STDOUT)
...
def get_full_info_peers(config_name, peers_id):
    """
    get downlofull info the wireguard peer.
    @param peers_id: Name of WG interfaces
    @type peers_id: array
    @return: array
    """
    result = []
    remove_files(DOWNLOAD_FILE_PATH)
...
@app.route('/full_download/<config_name>', methods=['GET'])
def full_download(config_name):
    """
    full download the wireguard peer.
    @param config_name: Name of WG interface
    @type config_name: str
    @return: redirects
    """
...
def init_dashboard():
    ...
    if 'dashboard_sort' not in config['Server']:
        config['Server']['dashboard_sort'] = 'name'
    # Default dashboard peers setting
    if "Peers" not in config:
        config['Peers'] = {}
    if 'peer_global_DNS' not in config['Peers']:
        config['Peers']['peer_global_DNS'] = '1.1.1.2, 1.0.0.2'
