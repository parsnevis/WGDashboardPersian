/**
 * configuration.js - Copyright(C) 2022 Mostafa Shoa
...
(function () {
    ...
    function configurationHeader(response) {
        ...
        if (response.checked === "checked") {
            ...
            $conf_status_btn.innerHTML = `<a href="#" id="${response.name}" ${response.checked} class="switch text-danger"><i class="bi bi-toggle2-off"></i> خاموش</a>`;
        }
        ...
        document.querySelector("#conf_total_user").innerHTML = `${response.peer_data.length}`;
        let active_user = 0;
        response.peer_data.forEach(function (currentValue, index) {
            if(currentValue.end_active)
                active_user += 1;
        })
        document.querySelector("#conf_total_active_user").innerHTML = `${active_user}`;
    }
        ...
        if (response.peer_data.length === 0) {
            ...
            response.peer_data.forEach(function (peer) {
                ...
                let html = '<div class="' + display_mode + '" data-id="' + peer.id + '">' +
                    '<div class="card mb-3 card-' + peer.status  + (peer.end_active == true ? '' : ' card-danger') + '">' +
                    '<div class="card-body">' +
                    '<div class="row">' +
                    peer_name +
                    peer_transfer +
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.message !== "true") {
                            $("#add_peer_alert").html(response.message).removeClass("d-none");
                            ...
                            $add_peer.removeAttribute("disabled");
                                $add_peer.innerHTML = "ذخیره کردن";
                                let link = '<a href="/downloads/'+response.data+'">لینک دانلود کاربران</a>'
                                window.configurations.showToast($new_add_amount.val()+" کاربر با موفقیت اضافه شد!"+link);
    ...
    window.configurations = {
        ...
        downloadOneImage: (conf) => {
            download_one_image(conf);
        },
        triggerIp: (ip) => {
            trigger_ip(ip);
        },
...
/**
 * When peer switch got click
 */
$body.on("click", ".switch-peer", function () {
    event.preventDefault()
    let selector = $(this);
    // selector.style.display = "none";
    // document.querySelector('div[role=status]').style.display = "inline-block";
    location.replace("/switch_peer/" + selector.data('id') + "?id=" + encodeURIComponent(selector.attr("id")));
});
// document.querySelector(".peer_list").addEventListener("click", (event) => {
//     event.preventDefault()
//     console.log(event.target)
//     let selector = document.querySelector(".switch-peer");
//     // console.log(selector.contains(event.target))
//     if (selector.contains(event.target)) {
//         // alert('sss')
//         selector.style.display = "none";
//         document.querySelector('div[role=status]').style.display = "inline-block";
//         location.replace(`/switch_peer/${selector.getAttribute("conf-name")}${selector.getAttribute("id")}`);
//     }
// });
